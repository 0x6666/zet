...
function setProgressBar(percents){
    jQuery("#bar").width(396*percents/100);
...
function showPromptNoImg(msgObj, callback, minHeight, defaultValue) {
	var option = {
		title : 'prompts',
		img : 'img/alert.png',
		msg : msgObj,
...
function getLast8Number(number){
	if(number.length > 11){
		return convertNumberToId(number.substring(number.length - 11, number.length));
	}
...
/**
 * 检查正在联网断网
 * "ppp_connecting" || "ppp_disconnecting"
 * @method checkConnectingOrDisconnectingStatus
 */
function checkConnectingOrDisconnectingStatus(currentConnStatus) {
	return currentConnStatus == "ppp_connecting" || currentConnStatus == "ppp_disconnecting" ;
}
...
/**
 * 弹出confirm_sbm提示框
 * @method showConfirm_sbm
 * @param {Object|String} msgObj
 * @param {Function} callback confirm确认后的回调函数
 * 		如果需要绑定Yes,No两个回调事件，传对象{ok: function(){}, no: function(){}}
 * @param {Number} minHeight
 */
function showConfirm_sbm(msgObj, callback, minHeight,yesTrans,noTrans) {
	if (yesTrans) {
		$('#yesbtn_sbm').attr("trans", yesTrans);
	} else {
		$('#yesbtn_sbm').attr("trans", "yes");
	}
	if (noTrans) {
		$('#nobtn_sbm').attr("trans", noTrans);
	} else {
		$('#nobtn_sbm').attr("trans", "no");
	}
...
function popup_sbm(option) {
	...
	var $confirmDiv = $('div#confirm_sbm');
	if(option.img==""){
		$('#confirmImg_sbm', $confirmDiv).hide();
	}else{
		$('#confirmImg_sbm', $confirmDiv).attr('src', option.img);
	}

	$('#popTitle_sbm', $confirmDiv).html($.i18n.prop(option.title));
	if (typeof option.msg == 'string') {
		$('.message', $confirmDiv).html($.i18n.prop(option.msg));
	} else {
		var params = [option.msg.msg];
	...
	if (option.showInput === true){
		$promptDiv.show();
		$("input#promptInput_sbm", $promptDiv).val(option.defaultValue ? option.defaultValue : "");
		$(".promptErrorLabel", $promptDiv).empty();
	} else {
		$promptDiv.hide();
	}
...
function shortenLongName(str,len){
	if(str.length > len){
