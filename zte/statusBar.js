...
define([ 'knockout', 'jquery', 'underscore', 'service', 'config/config', 'config/menu', 'tooltip'], function (ko, $, _, service, config, menu, tooltip) {
    ...
    function init() {
        ...
        window.setTimeout(function () {
            ...
            window.setInterval(function () {
				...
				vm.isShowConnectionIcon(config.DEVICE.toLowerCase().indexOf("cpe") ==-1 || (info.opms_wan_mode != "STATIC" && info.opms_wan_mode != "BRIDGE" && info.opms_wan_mode != "DHCP"));
                ...
                vm.batteryPers(convertBatteryPers(info.battery_value, info.batteryStatus));
                vm.batteryLevel(info.battery_value + '%');
				...
				if(config.HAS_SMS && info.sms_db_change == "1" && _smsInitComplete && !simStatusInvalid(info.simStatus)){
					service.getSmsUnreadNum({}, function(smsInfo){
						if(!_hasCheckedSmsCapacity && info.isLoggedIn){
							checkSmsCapacity(smsInfo.smsUnreadCount);
						} else {
							vm.smsUnreadCount(smsInfo.smsUnreadCount);
						}						
					});
                ...
                vm.wifi_5g_enable(info.wifi_5g_enable);
                checkWifi5GEnableStatus(vm.wifi_5g_enable());
			...
			var infoTemp = getStatusInfo();
			if(config.HAS_SMS && !simStatusInvalid(infoTemp.simStatus)){
				service.getSmsUnreadNum({}, function(smsInfo){
				if(!_hasCheckedSmsCapacity && infoTemp.isLoggedIn){
					checkSmsCapacity(smsInfo.smsUnreadCount);
				} else {
					vm.smsUnreadCount(smsInfo.smsUnreadCount);
				}
            ...
            window.setInterval(function() {
				...
				if(config.HAS_SMS && vm.isLoggedIn() && data.sms_db_change == "1"){
					checkSmsCapacity();
				}              
        ...
        checkTrafficLimitAlert = function(vm, info){
            ...
            var notPPP = (config.DEVICE.toLowerCase().indexOf("cpe") !=-1 && info.opms_wan_mode != "PPP") ? true : false;
    ...
    function statusViewModel() {
        ...
        self.batteryPers = ko.observable(convertBatteryPers(info.battery_value, info.batteryStatus));
        self.batteryLevel = ko.observable(info.battery_value + '%');
        ...
        self.wifi_5g_enable = ko.observable(info.wifi_5g_enable);
        checkWifi5GEnableStatus(self.wifi_5g_enable());
				...
				if(!isUpdate){
					...
					if(!config.currentChatObject || config.currentChatObject != getLast8Number(msg.number)){
						...
						if(newNum && newNum.text().length > 0){
							...
							newNum.text(1);
                            if(!item.hasClass("font-weight-bold")){
                                item.addClass("font-weight-bold");
                            }
                            item.find(".cursorhand").attr("title",tempNewSms);
                            item.find(".sms-table-content").html(tempNewSms);
						}
					}
				}
     ...
     * @param status
     */
    function convertBatteryPers(battery_val, status) {
        var src = null;
        ...
        if ("0" == status) {
            ...
            if (val >= 0 && val <= 5) {
                ...
                src = "img/battery_charging1.gif";
            } else if (val > 25 && val <= 50) {
                src = "img/battery_charging2.gif";
            } else if (val > 50 && val <= 90) {
                src = "img/battery_charging3.gif";
            } else{
                src = "img/battery_full.png";
            }
        }
    ...
    function checkWifi5GEnableStatus(wifi_5g_enable){
        if(wifi_5g_enable == '0'){
            var dirverCount = 0;
            function checkWifiDirver() {
                var result = service.getStatusInfo();
                if (result.radio_off=='0' || result.radio_off=='1') {
                    successOverlay();
                } else {
                    if (dirverCount <= 20) {
            ...
            service.getWifiAdvance({},function(data){
                if(data.wifiBand=='a'){
                    var params = {};
                    params.name = "wifi_5g_enable";
                    params.value = "1";
                    service.setNV(params, function(){
                        showConfirm("wifi_5g_enalbe_alert", function () {
                            showLoading();
                            service.setWifiBand({wifiEnabled: '1', wifi_band: 'b'}, function(result) {
                                if (result.result == "success") {
                                    setTimeout(function(){
                                        checkWifiDirver();
                                    },2000);
