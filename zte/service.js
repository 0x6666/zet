define([ 'underscore', 'jquery', 'config/config'], function(_, $, config) {
    ...
    function getStatusInfo(){
        if (timerInfo.isLoggedIn === undefined) {
            ...
            return {
		    	...
		    	sms_db_change: timerInfo.sms_db_change
    ...
    function getSmsCapability(){
		...
		function deal(data) {
            return {
				...
				nvUsed: parseInt(data.sms_nvused_total,10),//parseInt(data.sms_nv_rev_total,10) + parseInt(data.sms_nv_send_total,10) + parseInt(data.sms_nv_draftbox_total,10),
    ...
    'ssid', 'wifi_enable', 'wifi_5g_enable', 'check_web_conflict', 'dial_mode','wifi_onoff_func_control', 'ppp_dial_conn_fail_counter'];
    if(config.HAS_SMS){
        $.merge(loginTimerQueryString, ["sms_received_flag", "sts_received_flag","sms_db_change"]);
    }
    /**
     * 定时刷新临时回调列表
    ...
    function checkTimerUpdaterParameters() {
        ...
        if (window.location.hash && window.location.hash != '#login' && timerInfo.isLoggedIn) {
            if(config.HAS_SMS){
				...
				queryParams.sms_db_change_flag = 0;
            }
...
//        }
		...
		timerInfo.sms_db_change = data.sms_db_change;
...
//                currentLoginStatus = undefined;
	...
	function getSmsUnreadNum(){
		...
		function deal(data) {
			if (data) {
				return {smsUnreadCount: data.sms_unread_num};
			} else {
				return unknownErrorObject;
			}
		}
        ...
        function deal(data) {
            if (data) {
                return {
                	...
                	sw_version: data.wa_inner_version,
...
getBindIPInfo:getBindIPInfo,
	...
	delStaticAddrRules:delStaticAddrRules,
		...
		getSmsUnreadNum: getSmsUnreadNum,
